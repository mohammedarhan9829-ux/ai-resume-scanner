...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (btnSendOtp) {
        btnSendOtp.addEventListener("click", async () => {
            ...
            try {
                ...
                if (btnSubmitOtpReset) btnSubmitOtpReset.classList.remove("hidden");
            } catch (err) {
    ...
    if (formForgotPassword) {
        formForgotPassword.addEventListener("submit", async (e) => {
            e.preventDefault();
            const gmail = forgotPwdGmail.value.trim();
            const otp_code = forgotOtpCode ? forgotOtpCode.value.trim() : "";
            ...
            if (!otp_code || otp_code.length !== 6) {
                alert("⚠️ Please enter the 6-digit OTP verification code.");
                return;
            }
